...
#     # Flatten predictions in case it's not already flat
...
#     sorted_labels = sorted(zip(class_labels, predictions), key=lambda x: x[1], reverse=True)
    
#     # Display confidence scores
#     for index, (label, percentage) in enumerate(sorted_labels):
#         ax.text(
#             10,  # X position
#             25 + index * 25,  # Dynamically adjust Y position
#             f"{label}: {percentage * 100:.2f}%",
#             bbox=dict(facecolor="blue", alpha=0.5),  # Transparent background
#             fontsize=10,
#             color='white',
#         )
